        ...
        if fmin is scipy.optimize.fmin:
            ...
            if 11 < 3 and irestart == 0:  # toggle SLSQP
                output = scipy.optimize.fmin_slsqp(problem, propose_x0(),
                                ...
                                full_output=True, iprint=-1)
                stoppings[problem.index].append(output[3:])
            sigma = 2  # / 1.5**(irestart - 0)  # 1.6**9 = 70 used by Ilya # old: 0.2 if irestart == 0 else 2
            popsize = 2**irestart * default_popsize(problem.dimension)
            surrogate = inject_xopt = None  # use standard IPOP-CMA-ES by default
            if 11 < 3:  # sample sigma in a "safe" range
                sigma = 0.2 + 2 * np.random.rand()
                sigma = 2.2 * 10**-np.random.rand()
                sigma = 2.0 * 10**-np.random.rand()
            if 11 < 3:  # adaptive popsize
                if irestart < 1: ls_popsize = cma.optimization_tools.MultiplicativeConstStepSize1DSearch()
                else: ls_popsize.tell(es.result.fbest)
                popsize = ls_popsize.ask() * default_popsize(problem.dimension)
            if 11 < 3:  # model
            ...
            stoppings[problem.index].append(es.stop())
            if irestart >= 9:
                try: ls_popsize  # popsize linesearch adaptation is active
                except: break
                if popsize > 1e3 * default_popsize(problem.dimension):
