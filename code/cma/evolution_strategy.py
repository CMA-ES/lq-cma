...
class CMAEvolutionStrategy(interfaces.OOOptimizer):
    ...
    on by default only for very small population size only.
    ...
    *Selective mirroring* is implemented with option ``CMA_mirrors``
    ...
    Update: pairwise selection should probably also be applied to the
    covariance matrix update. For all injected solutions, as for those from
    TPA, this is now implemented in that for injected solutions the
    recombination weights are constrained to be nonnegative (for the
    covariance matrix, otherwise recombination weights are always
    nonnegative).
        ...
        if c1a + cmu > 0:
            ...
            for i, x in enumerate(pop):
                try:
                    ...
                    # sampler_weights[i + 1] = opts['CMA_active_injected']  # weight index 0 is for pc
